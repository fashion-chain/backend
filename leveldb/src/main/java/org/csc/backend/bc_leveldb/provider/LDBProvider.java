...
public class LDBProvider implements StoreServiceProvider, ActorService {
	...
	class DBStartThread extends Thread {
		...
		public void run() {
			try {
				...
				String loadpath = params.get("org.brewchain.leveldb.clib.path", "./clib/leveldb");
				LDBNative nativeLDB = null;
				try {
				...
				String dir = params.get("org.csc.ldb.dir",
	...
	public DomainDaoSupport getDaoByBeanName(DomainDaoSupport dds) {
		...
		if (dbi == null) {
			...
			String dir = params.get("org.csc.ldb.dir", "ldb." + Math.abs(NodeHelper.getCurrNodeListenOutPort() - 5100));
